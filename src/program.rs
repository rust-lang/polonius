...
use polonius_parser::{ir::Effect, parse_input};
        ...
        for (statement_idx, statement) in block.statements.iter().enumerate() {
            ...
            for effect in &statement.effects {
                match effect {
                    ...
                    Effect::Use { ref region } => {
                        // region_live_at: a `use` (in Mid points) emits a `region_live_at` fact at Start points
                        let region = tables.regions.intern(region);
                        facts.region_live_at.insert((region, start));
                    }

                    _ => emit_effect(&mut facts, effect, mid, tables),
                };
            }
...
fn emit_effect(facts: &mut Facts, effect: &Effect, point: Point, tables: &mut InternerTables) {
    match effect {
        // facts: borrow_region(Region, Loan, Point)
        Effect::BorrowRegionAt {
            ref region,
            ref loan,
        } => {
...
mod tests {
    ...
    fn complete_program() {
        ...
        assert_eq!(facts.region_live_at.len(), 1);
        {
